...
def track_motion_trajectory(
    ...
    *,
    ...
    if trained_model is not None:
        ...
        try:
            params = getattr(trained_model.adapter, "params", None)
        except Exception:
            params = None
        try:
    ...
    while True:
        ...
        # --- Initial hold: force first frame to 60/60 and log, then switch to normal control ---
        try:
            if getattr(controller, "_hold_init_once", False):
                a_pct = float(getattr(controller, "_init_valve_pct", 60.0))
                b_pct = float(getattr(controller, "_init_valve_pct", 60.0))
                # send to hardware
                try:
                    if dac is not None and hasattr(dac, "set_channels"):
                        a_pct_send = max(0.0, min(100.0, float(a_pct)))
                        b_pct_send = max(0.0, min(100.0, float(b_pct)))
                        try:
                            pct_to_code = getattr(dac, "_pct_to_code", None)
                            if callable(pct_to_code) and verbose:
                                a_code = int(pct_to_code(a_pct_send))
                                b_code = int(pct_to_code(b_pct_send))
                                print(f"[DEBUG] (init-hold) DAC codes -> a_code={a_code} b_code={b_code} (from a_pct={a_pct_send:.3f} b_pct={b_pct_send:.3f})", flush=True)
                        except Exception:
                            pass
                ...
                row: list[float | int | str] = [ms, a_pct, b_pct, qdes, ""]
                if adc_vals:
                    row.extend(adc_vals)
                if ldc_vals:
                    row.extend(ldc_vals)
                ...
                try:
                    setattr(controller, "_hold_init_once", False)
                except Exception:
                    pass
                continue
        except Exception:
        ...
        if trained_model is not None:
            ...
            # Build states for adapter (only target slot is filled with measurements; others 0)
            ...
            # Optional tension
            states = {"q": q_vec, "dq": dq_vec, "pa": pa_vec, "pb": pb_vec}
            if include_tension:
                ta_vec = np.zeros((n_full,), dtype=float); ta_vec[target_joint_id] = float(ta)
                tb_vec = np.zeros((n_full,), dtype=float); tb_vec[target_joint_id] = float(tb)
                states["Ta"] = ta_vec
                states["Tb"] = tb_vec
            try:
                X = trained_model.adapter.make_model_input(
                    t,
                    states,
                    {"qdes": qdes_vec_func, "dqdes": dqdes_vec_func},
                )
                # Validate input feature size strictly matches scaler expectation (no pad/trunc)
                if expected_n_features is not None:
                    nf = int(expected_n_features)
                    if getattr(X, 'ndim', 2) != 2 or X.shape[1] != nf:
                        # force fallback to manual construction
                        raise RuntimeError("feature-size-mismatch")
            except Exception:
                # Build manual feature vector: [q, dq, pa, pb, (Ta, Tb), qdes, (dqdes)] per active_joints
                params = getattr(trained_model.adapter, 'params', None)
                try:
                    joints = list(getattr(params, 'active_joints', [target_joint_id])) if params is not None else [target_joint_id]
                except Exception:
                    joints = [target_joint_id]
                try:
                ...
                if include_tension:
                    taj = (ta_vec if 'ta_vec' in locals() else np.zeros_like(qj))
                    tbj = (tb_vec if 'tb_vec' in locals() else np.zeros_like(qj))
                    taj = taj[joints] if taj.shape[0] >= max(joints)+1 else np.zeros_like(qj)
                    tbj = tbj[joints] if tbj.shape[0] >= max(joints)+1 else np.zeros_like(qj)
                    feats.extend([taj, tbj])
                qdes_prev = qdes_vec_func(t + preview_time)[joints]
                feats.append(qdes_prev)
                if include_dqdes:
                    dqdes_prev = dqdes_vec_func(t + preview_time)[joints]
                    feats.append(dqdes_prev)
                model_input = np.concatenate(feats)
                X = np.atleast_2d(model_input)
                # Final validation against scaler expectation
                if expected_n_features is not None:
                    nf = int(expected_n_features)
                    if X.shape[1] != nf:
                        if verbose:
                            print(f"[WARN] Manual feature size mismatch: X.shape={X.shape} != expected {nf}. Falling back to PID.", flush=True)
                        raise RuntimeError("manual-feature-size-mismatch")
                if _debug_left > 0 and verbose:
                    print(f"[DEBUG] Built manual X to match training layout: shape={X.shape}", flush=True)
            if _debug_left > 0 and verbose:
                print(f"[DEBUG] X_shape={getattr(X,'shape',None)} expected_n_features={expected_n_features}", flush=True)
            ...
            # Prefer adapter.make_ctrl_input to map model outputs to per-joint arrays.
            # Treat adapter/model outputs as raw control values (0..5 range) and scale to percent.
            a_pct = b_pct = None
            a_raw = b_raw = None
            ...
            RAW_MAX = 5.0
            try:
                ca, cb = trained_model.adapter.make_ctrl_input(y_arr, {"ca": ca, "cb": cb})
                a_raw = float(ca[target_joint_id]); b_raw = float(cb[target_joint_id])
                a_pct = float(np.clip(a_raw, 0.0, RAW_MAX) / RAW_MAX * 100.0)
                b_pct = float(np.clip(b_raw, 0.0, RAW_MAX) / RAW_MAX * 100.0)
            except Exception:
                # Fallback: if model returns two values, interpret them as raw [a,b] and scale to percent
                try:
                    if y_arr.shape[1] >= 2:
                        a_raw = float(y_arr[0, 0]); b_raw = float(y_arr[0, 1])
                        a_pct = float(np.clip(a_raw, 0.0, RAW_MAX) / RAW_MAX * 100.0)
                        b_pct = float(np.clip(b_raw, 0.0, RAW_MAX) / RAW_MAX * 100.0)
                    else:
                        a_pct = 0.0; b_pct = 0.0
            ...
            if _debug_left > 0 and verbose:
                ...
                try:
                    ...
                    if a_raw is not None and b_raw is not None:
                        print(f"[DEBUG] a_raw={a_raw:.3f} b_raw={b_raw:.3f}", flush=True)
...
def main() -> None:
    ...
    try:
        if dac is not None:
            # Start valves at 60% to match historic behaviour
            init_valve_pct = 60.0
            ...
            # expose to controller for first-frame hold
            try:
                setattr(ctrl, "_init_valve_pct", float(init_valve_pct))
                setattr(ctrl, "_hold_init_once", True)
            except Exception:
                pass
        time.sleep(1.0)
        ...
        if enc_zero is None:
            enc_zero = 0.0
        # wait 1s after zero capture (align with request)
        time.sleep(1.0)
    except Exception:
        if enc_zero is None:
            enc_zero = 0.0
    ...
    try:
        ...
        try:
            ...
            prev_step = getattr(trained_model.adapter.params, "preview_step", None)
            dt_param = getattr(trained_model.adapter.params, "dt", None)
            inc_dqdes = getattr(trained_model.adapter.params, "include_dqdes", None)
        except Exception:
            aj = dof = inc_t = ang_u = prev_step = dt_param = inc_dqdes = None
        ...
        try:
            ...
            pass
        print(
            f"[INFO] Model loaded: {getattr(trained_model, 'model_name', type(trained_model).__name__)} (active_joints={aj}, dof={dof}, include_tension={inc_t}, angle_unit={ang_u}, preview_step={prev_step}, dt={dt_param}, include_dqdes={inc_dqdes}, expected_n_features={expected_n_features})",
            flush=True,
        )
    except Exception as e:
