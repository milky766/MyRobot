...
def plot_csv(csv_path: str) -> str:
    ...
    label_map = {
        ...
        'enc_deg': 'q', 'enc': 'q',
        'valve_a_pct': 'ca', 'ca': 'ca',
        'valve_b_pct': 'cb', 'cb': 'cb',
        'adc0_kpa': 'pa', 'pa': 'pa',
        'adc1_kpa': 'pb', 'pb': 'pb',
        'tension_0x2B': 'Tb',
        'tension_0x2A': 'Ta',
    ...
    ax1.plot(sec, enc, '-x', color='red', linewidth=1.5, markersize=6, label=label_map.get('q', 'q'))
    ax1.plot(sec, q_des, '-o', color='tab:green', linewidth=1.5, markersize=6, label=label_map.get('qdes', 'qdes'))
    ...
    # user requested: ca -> 黄色, cb -> 青
    # Plot available valve series (prefer raw DAC 'ca'/'cb', otherwise percentage columns)
    ax3.plot(sec, a, '-o', color='gold', label=label_map.get('ca', 'ca'))
    ax3.plot(sec, b, '-o', color='tab:blue', label=label_map.get('cb', 'cb'))
    ax3.legend(loc='upper right')
    ax3.set_title('Valve Command')
    ax3.yaxis.set_major_locator(mticker.MaxNLocator(nbins=5))

    # Pressure plot
    ax4 = fig.add_subplot(n_subplots, 1, 4)
    # user requested: (pa,pb) = (黄色, 青)
    if p0 is not None:
        ax4.plot(sec, p0, '-o', color='gold', label=label_map.get('pa', 'pa'))
    if p1 is not None:
        ax4.plot(sec, p1, '-o', color='tab:blue', label=label_map.get('pb', 'pb'))
